...
for ((i=1; i<=$1; i++)); do
  result=$(opencode run "@PRD.md @progress.md \
  1. Read the PRD.md and progress.md files. \
  ...
  If the progress is complete, output <promise>COMPLETE</promise>." -m "github-copilot/claude-sonnet-4.6")
